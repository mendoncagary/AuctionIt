...
angular.module('AuctionIt.controllers', []).controller('IndexController', ["$scope", "$location", "socket", "$route", function ($scope, $location, socket, $route) {
  ...
  $(document).ready(function () {
    ...
    $('#clist').click(function (event) {
      event.preventDefault();$('#cproducts .item').addClass('list-group-item');
    });
    $('#cgrid').click(function (event) {
      event.preventDefault();$('#cproducts .item').removeClass('list-group-item');$('#cproducts .item').addClass('grid-group-item');
    });
  });
  ...
  $http.get('/api/item/' + $routeParams.id).then(function (success) {
  ...
  $http.get('/api/profile/').then(function (success) {
    $scope.user = success.data;
  }, function (error) {});

  $scope.inv_message = "You have no items in your inventory";

  $http.get('/api/itemswon').then(function (response) {
    $scope.inv_message = "";
    $scope.items_won = [];
  ...
  $http.get('/api/badges').then(function (response) {
  ...
  $("#input-1").rating({
    ...
    $http.put('/api/rating', JSON.stringify({ value: value }), 'application/json').then(function (response) {
